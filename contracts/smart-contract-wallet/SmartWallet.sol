    ...
    ReentrancyGuardUpgradeable
{
    ...
    // onlyOwnerOrOthers
    /**
     * @notice Throws if the sender is not owner or from other addresses.
     */
    modifier onlyOwnerOrOthers() {
        require(isOwner() || isOtherAddresses(), "Authorization failed");
        _;
    }
    ...
    function isOtherAddresses() public returns (bool check) {
        uint256 leng = others.length;
        for (uint256 i; i < leng; ) {
            if (msg.sender == others[i]) {
                check = true;
                break;
            } else {
                check = false;
    ...
    function transfer(address payable dest, uint256 amount)
        external
        nonReentrant
        onlyOwnerOrOthers
    {
    ...
    function _payPrefund(uint256 requiredPrefund) internal override {
